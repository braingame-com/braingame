...
const FirebaseSchema = z.object({
	...
	FIREBASE_AUTH_DOMAIN: z.string().regex(/^[\w-]+\.firebaseapp\.com$/, "Invalid Firebase auth domain"),
	FIREBASE_PROJECT_ID: z.string().min(1, "Firebase project ID is required"),
	FIREBASE_STORAGE_BUCKET: z.string().regex(/^[\w-]+\.appspot\.com$/, "Invalid Firebase storage bucket"),
	FIREBASE_MESSAGING_SENDER_ID: z.string().regex(/^\d+$/, "Firebase messaging sender ID must be numeric"),
 ...
 */
export const ProductAppEnvSchema = BaseEnvSchema
...
export const WebsiteEnvSchema = z.object({
	...
	NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN: z.string().regex(/^[\w-]+\.firebaseapp\.com$/, "Invalid Firebase auth domain"),
	NEXT_PUBLIC_FIREBASE_PROJECT_ID: z.string().min(1, "Firebase project ID is required"),
	NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET: z.string().regex(/^[\w-]+\.appspot\.com$/, "Invalid Firebase storage bucket"),
	NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID: z.string().regex(/^\d+$/, "Firebase messaging sender ID must be numeric"),
