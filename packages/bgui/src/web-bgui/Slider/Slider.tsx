...
import type { SliderOwnerState, SliderTypeMap } from "./SliderProps";
	...
	return {
		...
		...(ownerState.orientation === "horizontal" && {
			...
			...(ownerState.percent === 100 && {
				transform:
					"translate(calc(var(--Slider-markSize) * -1 - min(var(--Slider-markSize), 3px)), -50%)",
			}),
		}),
		...(ownerState.orientation === "vertical" && {
			left: "50%",
			...
			...(ownerState.percent === 100 && {
				transform:
					"translate(-50%, calc(var(--Slider-markSize) * 1 + min(var(--Slider-markSize), 3px)))",
			}),
		}),
	};
});
