...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Buttons trigger actions or events when activated. They communicate calls to action to the user and allow them to interact with pages.',
      },
    },
  },
  argTypes: {
      ...
      <div>
        {variants.map(variant => (
          <div key={variant} style={{ marginBottom: 20 }}>
            <h3 style={{ marginBottom: 10 }}>{variant.charAt(0).toUpperCase() + variant.slice(1)}</h3>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {colors.map(color => (
