...
const ErrorUI = ({ error }: { error: Error | null }) => {
	...
	const { errorContainerStyle, errorTitleStyle, errorMessageStyle } = getErrorStyles(colors);
	...
	return (
		...
		>
			...
			<Text style={errorMessageStyle}>{error?.message || "An unexpected error occurred"}</Text>
		</View>
	);
};
...
> {
	...
	render() {
		if (this.state.hasError) {
			...
			// Default error UI - wrap in ThemeProvider to ensure theme access
			return (
				<ThemeProvider>
